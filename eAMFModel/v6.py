"""v6: prices moneyline, spread and total from inside a game by simulating the rest of it play by
play."""
...
class Variant:
    """One way of running v6 (what it reacts to in the game)."""
        ...
        if len(sides) >= 200:
            ...
            if verbose:
                ...
                print(f"  form on the day, sd in log points: the game {np.sqrt(tables.strength_game):.3f}"
                      ...
                      f" with no history)")
    tables_path = os.path.join(out_dir, "v6tables.npz")
    grid_path = os.path.join(out_dir, "v6grid.npz")
...
def players_book(model_dir_or_tables):
    ...
    import os
    d = model_dir_or_tables if os.path.isdir(model_dir_or_tables) else os.path.dirname(model_dir_or_tables)
    path = os.path.join(d, "v6players.json")
...
def run(path, tables_path, grid_path, variants, matches=None, n_paths=1000, workers=4,
    ...
    """Price an export with v6 and grade it against the results."""
    if book is None:
    ...
    if priors is None and history is not None:
        if pre is None:
            raise SystemExit("this v6 model has no NB2 pre-match model: rebuild it with --history")
        ...
        raise SystemExit("this v6 model prices pre-match with NB2: pass --history (the matches'"
